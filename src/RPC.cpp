...
static int RPC_ReadMessage()
{
	...
	if (g_np.offline) {
		char cacheFile[255];
		_snprintf(cacheFile, sizeof(cacheFile), ".\\cache\\%d.cache", g_rpc.messageType);
		std::ifstream infile(cacheFile, std::ifstream::binary);
		if (infile.is_open())
		{
			infile.seekg(0, infile.end);
			long size = infile.tellg();
			infile.seekg(0);
			Log_Print("Offline emulating response from: %s %d\n", cacheFile, size);
			infile.read((char*)buffer, size);
			len = 1;
		}
		else {
			Log_Print("Missing offline files...\n");
			return FD_CLOSE;
		}
	} else {
		len = recv(g_rpc.socket, (char*)buffer, sizeof(buffer), 0);
	}
		...
		while (read > 0)
		{
			...
			if (g_rpc.readBytes == 0)
			{
				...
				if (!g_np.offline) {
					...
					char cacheFile[255];
					_snprintf(cacheFile, sizeof(cacheFile), ".\\cache\\%d.cache", g_rpc.messageType);
					std::ofstream outfile(cacheFile, std::ofstream::binary);
...
{
	if (g_rpc.connected || g_np.offline == true)
	{
		return true;
	}

	//Cache folder is for the offline response
	mkdir("cache");

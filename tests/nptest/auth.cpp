...
bool Auth_DisplayLoginDialog(const wchar_t** usernameP, const wchar_t** passwordP)
{
	...
	info.pszMessageText = L"Please log in to use NPTest. Use your RepZ forum credentials.";
...
bool Auth_PerformSessionLogin(const char* username, const char* password)
{
	...
	{
		char url[255];
		_snprintf(url, sizeof(url), "http://%s/devauthenticate", "server.repziw4.de");//authenticate live np, devauthenticate for dev np

		char buf[8192] = {0};
		char postBuf[8192];
		_snprintf(postBuf, sizeof(postBuf), "%s&&%s&&s", username, password, "nptest");

		curl_easy_setopt(curl, CURLOPT_URL, url);
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, DataReceived);
		curl_easy_setopt(curl, CURLOPT_WRITEDATA, (void*)&buf);
		curl_easy_setopt(curl, CURLOPT_USERAGENT, "npTest");
		...
		{
			Auth_Error(va("Could not reach the repziw4 server. Error code from CURL: %x.", code));
		}
...
void Auth_VerifyIdentity()
{
	...
	printf("Logging in...");
	bool result = Auth_PerformSessionLogin("iain17", "y05Zn2YoBVm5ziBXQlQxjll9adE8bz2CEBk8jlguvPp9d86CTDXREpsHZY4iNLyn");
	printf("passed this part\n");
	if (result)
	{
		return;
	}
